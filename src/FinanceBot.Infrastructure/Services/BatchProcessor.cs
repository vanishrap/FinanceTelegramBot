...
public sealed class BatchProcessor(IDbContextFactory<FinanceDbContext> factory, ITelegramClient telegram, IVoiceTranscriptionService transcription, IAiExtractionService extraction, IAiAnalyticsService analytics, IAnalyticsQueryExecutor queryExecutor, FinanceOptions options, ILogger<BatchProcessor> logger)
{
 ...
 private async Task ProcessAsync(long id,CancellationToken ct)
 {
  ...
  try
  {
   ...
   var output=await extraction.ExtractAsync(aiInput,ct); logger.LogDebug("OpenAI extracted operation for InputBatchId={InputBatchId}: {Output}",id,output); run.OutputJson=output; var operation=JsonSerializer.Deserialize<ExtractedOperation>(output,JsonOptions) ?? throw new InvalidDataException("Empty AI operation.");
   ...
   if(operation.Kind is "Expense" or "Income") checks.Add(new("Category",1,category is null?0:1,category is not null,"Уточните категорию операции или напишите, на что именно были потрачены деньги."));
   ...
   if((operation.Kind is "Expense" or "Income") && !hasSpecificPurpose) checks.Add(new("Purpose",1,0,false,"Уточните, на что именно были потрачены деньги или от кого и за что получен доход."));
   ...
   Account? selectedAccount=operation.AccountId is long accountId ? await db.Accounts.SingleOrDefaultAsync(x=>x.Id==accountId && x.IsActive && (x.OwnerUserId==null || x.OwnerUserId==batch.UserId),ct) : null;
   if((operation.Kind is "Expense" or "Income" or "BalanceAdjustment" or "DebtSettlement") && selectedAccount is null) checks.Add(new("Account",1,0,false,"Не удалось определить счёт, с которого нужно провести операцию."));
   ...
   if(operation.Kind=="Transfer" && (selectedAccount is null || toAccount is null)) checks.Add(new("TransferAccounts",2,(selectedAccount is null?0:1)+(toAccount is null?0:1),false,"Для перевода нужны исходный и целевой счета."));
   ...
   if(operation.Kind=="DebtCreate")
   {
    if(operation.Amount<=0) checks.Add(new("DebtAmount",1,operation.Amount,false,"Уточните положительную сумму долга."));
    if(string.IsNullOrWhiteSpace(operation.Counterparty)) checks.Add(new("DebtCounterparty",1,0,false,"Уточните, кому вы должны или кто должен вам."));
    if(!Enum.TryParse<DebtDirection>(operation.DebtDirection,true,out _)) checks.Add(new("DebtDirection",1,0,false,"Уточните направление долга: вы должны или должны вам."));
   }
   if(operation.Kind=="Correction") checks.Add(new("UnsupportedOperation",0,1,false,"Operation requires review."));
   ...
   if(checks.Any(x=>!x.Passed)){batch.Status=BatchStatus.NeedsReview;run.Status=RunStatus.Completed;run.CompletedAt=DateTimeOffset.UtcNow;await db.SaveChangesAsync(ct);var questions=checks.Where(x=>!x.Passed).Select(x=>x.Message).Distinct();await NotifyAsync(batch.User.TelegramUserId,$"⚠️ Пока не могу записать операцию ({operation.Amount:0.00} {operation.Currency}).\n\n{string.Join("\n",questions.Select(x=>$"• {x}"))}\n\nПожалуйста, уточните недостающие данные ответным сообщением — я сохраню контекст и попробую снова.",ct);return;}
   var operationDate=operation.TransactionDate ?? DateTimeOffset.UtcNow.ToOffset(MalaysiaOffset);
   if(operation.Kind=="DebtCreate")
   {
    var direction=Enum.Parse<DebtDirection>(operation.DebtDirection!,true);var debt=new Debt{CreatedByUserId=batch.UserId,Direction=direction,Counterparty=operation.Counterparty!.Trim(),CurrencyCode=operation.Currency,OriginalAmount=operation.Amount,Description=operation.Description,CreatedAt=operationDate,Status=DebtStatus.Open,InputBatchId=id};db.Debts.Add(debt);batch.Status=BatchStatus.Completed;batch.CompletedAt=DateTimeOffset.UtcNow;run.Status=RunStatus.Completed;run.CompletedAt=DateTimeOffset.UtcNow;await db.SaveChangesAsync(ct);var relation=direction==DebtDirection.Payable?$"Вы должны: {debt.Counterparty}":$"Вам должен(на): {debt.Counterparty}";await NotifyAsync(batch.User.TelegramUserId,$"✅ Долг записан\n\n{relation}\nСумма: {debt.OriginalAmount:0.00} {debt.CurrencyCode}\nДата: {debt.CreatedAt:dd.MM.yyyy HH:mm zzz}\nОписание: {debt.Description}\n\nID долга: {debt.Id}",ct);return;
   }
   await using var tx=await db.Database.BeginTransactionAsync(ct); var now=DateTimeOffset.UtcNow; var transactionDate=operationDate; var type=Enum.Parse<TransactionType>(operation.Kind); var transaction=new Transaction{CreatedByUserId=batch.UserId,Type=type,TransactionDate=transactionDate,CurrencyCode=operation.Currency,Amount=operation.Amount,Description=operation.Description,MerchantName=operation.Merchant,CategoryId=category?.Id,InputBatchId=id,CreatedAt=now,UpdatedAt=now};
   if(selectedAccount is not null)
   {
    var movement=type==TransactionType.BalanceAdjustment ? operation.Amount-(await db.AccountMovements.Where(x=>x.AccountId==selectedAccount.Id).SumAsync(x=>(decimal?)x.Amount,ct)??0) : type is TransactionType.Expense or TransactionType.DebtSettlement ? -operation.Amount : operation.Amount; transaction.Movements.Add(new(){Account=selectedAccount,Amount=movement});
   }
   if(type==TransactionType.Transfer && toAccount is not null) transaction.Movements.Add(new(){Account=toAccount,Amount=operation.Amount});
