...
public sealed class BatchProcessor(IDbContextFactory<FinanceDbContext> factory, ITelegramClient telegram, IVoiceTranscriptionService transcription, IAiExtractionService extraction, IAiAnalyticsService analytics, IAnalyticsQueryExecutor queryExecutor, FinanceOptions options, ILogger<BatchProcessor> logger)
{
 ...
 private async Task ProcessAsync(long id,CancellationToken ct)
 {
  ...
  try
  {
   ...
   var validator=new ValidationService(options.ValidationRoundingTolerance);var receiptCorrectionAttempts=0;
  validateExtraction:
   var operations=ReadOperations(output);
   ...
   foreach(var operation in operations)
   {
    ...
    var checks=operation.Kind=="Delete"?new List<ValidationCheck>():validator.Validate(operation).ToList();
    ...
    if(operation.Receipt is{} operationReceipt)foreach(var receiptItem in operationReceipt.Items.Where(x=>x.FinalAmount>0))
    {
     var itemCategory=categories.SingleOrDefault(x=>x.Id==receiptItem.CategoryId&&x.Type==CategoryType.Expense);
     if(itemCategory is null||itemCategory.ParentId is null)checks.Add(new("ReceiptItemCategory",1,0,false,$"Для позиции «{receiptItem.Name}» выберите отдельную подходящую подкатегорию по назначению товара, а не общую категорию магазина или «Покупки»."));
    }
   ...
   var failedChecks=prepared.SelectMany(x=>x.Checks).Where(x=>!x.Passed).ToList();
   var receiptFailures=failedChecks.Where(x=>AutoCorrectableReceiptChecks.Contains(x.Type)).ToList();
   if(receiptFailures.Count>0&&receiptCorrectionAttempts<2)
   {
    receiptCorrectionAttempts++;
    var previousExtraction=JsonSerializer.Deserialize<JsonElement>(output,JsonOptions);
    var feedback=JsonSerializer.Serialize(new{instruction="Deterministic receipt validation failed. Re-read the original messages and images, correct the arithmetic or extracted receipt values, preserve every original operation and return the complete corrected schema JSON. Do not ask the user to calculate anything.",attempt=receiptCorrectionAttempts,validationErrors=receiptFailures.Select(x=>new{x.Type,x.Expected,x.Actual,x.Message}),previousExtraction},JsonOptions);
    logger.LogWarning("Automatically retrying receipt extraction for InputBatchId={InputBatchId}, Attempt={Attempt}, Feedback={Feedback}",id,receiptCorrectionAttempts,feedback);
    foreach(var result in run.ValidationResults)db.Entry(result).State=EntityState.Detached;run.ValidationResults.Clear();
    output=await extraction.ExtractAsync(new AiInput([..texts,$"[AUTOMATIC VALIDATION FEEDBACK] {feedback}"],images,context),ct);run.OutputJson=output;
    goto validateExtraction;
   }
   var failures=failedChecks.Where(x=>!AutoCorrectableReceiptChecks.Contains(x.Type)).Select(x=>x.Message).Distinct().ToList();
   if(receiptFailures.Count>0)failures.Add("Не удалось автоматически перепроверить арифметику чека после двух попыток. Исходный чек и контекст сохранены; попробуйте обработать его повторно позже.");
   ...
   for(var operationIndex=0;operationIndex<prepared.Count;operationIndex++)
   {
    ...
    if(operation.Kind=="Correction")
    {
     ...
     if(operation.Receipt is{} correctedReceipt)
     {
      var storedReceipt=await db.Receipts.Include(x=>x.Items).SingleOrDefaultAsync(x=>x.TransactionId==target.Id,ct);
      if(storedReceipt is null){storedReceipt=new(){Transaction=target};db.Receipts.Add(storedReceipt);}
      else {db.ReceiptItems.RemoveRange(storedReceipt.Items);storedReceipt.Items.Clear();}
      storedReceipt.MerchantName=operation.Merchant??operation.Description;storedReceipt.ReceiptDate=target.TransactionDate;storedReceipt.Subtotal=correctedReceipt.Subtotal;storedReceipt.Tax=correctedReceipt.Tax;storedReceipt.ServiceCharge=correctedReceipt.ServiceCharge;storedReceipt.Discount=correctedReceipt.Discount;storedReceipt.Rounding=correctedReceipt.Rounding;storedReceipt.Total=correctedReceipt.Total;storedReceipt.CurrencyCode=target.CurrencyCode;storedReceipt.RawExtractedJson=output;storedReceipt.Items=CreateReceiptItems(correctedReceipt);
     }
    ...
    if(operation.Kind=="DebtToExpense")
    {
     var debt=item.TargetDebt!;var oldDebtJson=SerializeDebt(debt);var debtExpenseTransaction=new Transaction{CreatedByUserId=batch.UserId,Type=TransactionType.Expense,TransactionDate=debt.CreatedAt,CurrencyCode=debt.CurrencyCode,Amount=debt.OriginalAmount,Description=debt.Description,MerchantName=operation.Merchant,CategoryId=item.Category!.Id,InputBatchId=id,InputBatchOperationIndex=operationIndex,CreatedAt=now,UpdatedAt=now,Movements=[new AccountMovement{Account=item.Account!,Amount=-debt.OriginalAmount}]};db.Transactions.Add(debtExpenseTransaction);debt.Status=DebtStatus.Cancelled;db.AuditLog.Add(new(){EntityType=nameof(Debt),EntityId=debt.Id,Action="ReclassifyToExpense",OldJson=oldDebtJson,NewJson=SerializeDebt(debt),UserId=batch.UserId,CreatedAt=now});saved.Add((operation,debtExpenseTransaction,debt,item.Category,item.Account));continue;
    }
    ...
    var type=Enum.Parse<TransactionType>(operation.Kind);var newTransaction=new Transaction{CreatedByUserId=batch.UserId,Type=type,TransactionDate=operationDate,CurrencyCode=operation.Currency,Amount=operation.Amount,Description=operation.Description,MerchantName=operation.Merchant,CategoryId=item.Category?.Id,InputBatchId=id,InputBatchOperationIndex=operationIndex,CreatedAt=now,UpdatedAt=now};
    if(item.Account is not null){var movement=type==TransactionType.BalanceAdjustment?operation.Amount-(await db.AccountMovements.Where(x=>x.AccountId==item.Account.Id).SumAsync(x=>(decimal?)x.Amount,ct)??0):type is TransactionType.Expense or TransactionType.DebtSettlement?-operation.Amount:operation.Amount;newTransaction.Movements.Add(new(){Account=item.Account,Amount=movement});}
    if(type==TransactionType.Transfer&&item.ToAccount is not null)newTransaction.Movements.Add(new(){Account=item.ToAccount,Amount=operation.Amount});
    var movementCheck=validator.ValidateMovements(operation,newTransaction.Movements.Select(x=>x.Amount).ToList());run.ValidationResults.Add(new(){Type=movementCheck.Type,ExpectedValue=movementCheck.Expected,ActualValue=movementCheck.Actual,Difference=movementCheck.Actual-movementCheck.Expected,Status=movementCheck.Passed?ValidationStatus.Pass:ValidationStatus.Fail,Message=movementCheck.Message});if(!movementCheck.Passed)throw new InvalidDataException($"Invalid movements for {operation.Description}.");
    db.Transactions.Add(newTransaction);if(operation.Receipt is{} receipt)db.Receipts.Add(new(){Transaction=newTransaction,MerchantName=operation.Merchant??operation.Description,ReceiptDate=operationDate,Subtotal=receipt.Subtotal,Tax=receipt.Tax,ServiceCharge=receipt.ServiceCharge,Discount=receipt.Discount,Rounding=receipt.Rounding,Total=receipt.Total,CurrencyCode=operation.Currency,RawExtractedJson=output,Items=CreateReceiptItems(receipt)});
    saved.Add((operation,newTransaction,null,item.Category,item.Account));
   }
   ...
   else if(saved.Count==1){var single=saved[0];var categoryPath=single.Category is null?"Без категории":single.Category.Parent is null?single.Category.Name:$"{single.Category.Parent.Name} → {single.Category.Name}";confirmation=$"✅ Транзакция записана\n\nИтоговая сумма: {single.Operation.Amount:0.00} {single.Operation.Currency}{FormatReceiptBreakdown(single.Operation.Receipt,single.Operation.Currency)}\nДата: {single.Transaction!.TransactionDate:dd.MM.yyyy HH:mm zzz}\nКатегория: {categoryPath}\nОписание: {single.Operation.Merchant??single.Operation.Description}{(single.Account is null?"":$"\nСчёт: {single.Account.Name}")}\n\nID операции: {single.Transaction.Id}";}
   else {var lines=saved.Select((x,i)=>x.Debt is not null?$"{i+1}. **Долг {x.Debt.OriginalAmount:0.00} {x.Debt.CurrencyCode}** — {(x.Debt.Direction==DebtDirection.Payable?"вы должны":"вам должен(на)")} {x.Debt.Counterparty} (ID {x.Debt.Id})":$"{i+1}. **{x.Operation.Amount:0.00} {x.Operation.Currency}** — {x.Operation.Merchant??x.Operation.Description}, {x.Category?.Name??"без категории"}{(x.Account is null?"":$", счёт {x.Account.Name}")} (ID {x.Transaction!.Id}){FormatReceiptBreakdown(x.Operation.Receipt,x.Operation.Currency)}");confirmation=$"✅ **Записано операций: {saved.Count}**\n\n{string.Join("\n",lines)}";}
  }
 ...
 private static string FormatReceiptBreakdown(ReceiptDraft? receipt,string currency)
 {
  if(receipt is null)return "";
  var items=receipt.Items.Select((item,index)=>$"{index+1}. {item.Name} — {item.Quantity:0.##} × {item.UnitPrice:0.00} {currency}; итог: {item.FinalAmount:0.00} {currency}");
  var itemsTotal=receipt.Items.Sum(x=>x.FinalAmount);
  return $"\n\nПозиции ({receipt.Items.Count}):\n{string.Join("\n",items)}\n\nПроверка: сумма позиций {itemsTotal:0.00} {currency} совпадает с итогом ✅\n";
 }
 private static List<ReceiptItem> CreateReceiptItems(ReceiptDraft receipt) => receipt.Items.Select(x=>new ReceiptItem{RawName=x.Name,NormalizedName=x.Name,Quantity=x.Quantity,UnitPrice=x.UnitPrice,BaseAmount=x.BaseAmount,Discount=x.Discount,TaxAllocated=x.TaxAllocated,ServiceChargeAllocated=x.ServiceChargeAllocated,FinalAmount=x.FinalAmount,CategoryId=x.CategoryId,AiConfidence=x.Confidence}).ToList();
 ...
 private static bool IsAnalyticsResultEmpty(string json)
 {
  using var document=JsonDocument.Parse(json);if(!document.RootElement.TryGetProperty("rows",out var rows)||rows.GetArrayLength()==0)return true;
  var metrics=rows.EnumerateArray().SelectMany(x=>x.EnumerateObject()).Where(x=>x.Name.Contains("amount",StringComparison.OrdinalIgnoreCase)||x.Name.Contains("total",StringComparison.OrdinalIgnoreCase)||x.Name.Contains("count",StringComparison.OrdinalIgnoreCase)||x.Name.Contains("sum",StringComparison.OrdinalIgnoreCase)).Select(x=>x.Value).ToList();
  return metrics.Count>0&&metrics.All(x=>x.ValueKind is JsonValueKind.Null||x.ValueKind==JsonValueKind.Number&&x.TryGetDecimal(out var value)&&value==0||x.ValueKind==JsonValueKind.String&&decimal.TryParse(x.GetString(),System.Globalization.NumberStyles.Number,System.Globalization.CultureInfo.InvariantCulture,out var parsed)&&parsed==0);
 }
