...
public sealed class BatchProcessor(IDbContextFactory<FinanceDbContext> factory, ITelegramClient telegram, IVoiceTranscriptionService transcription, IAiExtractionService extraction, FinanceOptions options, ILogger<BatchProcessor> logger)
{
 ...
 private async Task ProcessAsync(long id,CancellationToken ct)
 {
  ...
  try
  {
   ...
   batch.Status=BatchStatus.Completed;batch.CompletedAt=now;run.Status=RunStatus.Completed;run.CompletedAt=now;await db.SaveChangesAsync(ct);await tx.CommitAsync(ct);logger.LogDebug("Database write committed: TransactionId={TransactionId}, InputBatchId={InputBatchId}, Type={Type}, Amount={Amount}, Currency={Currency}, Description={Description}, Merchant={Merchant}, AccountMovements={AccountMovements}, ReceiptId={ReceiptId}, AiRunId={AiRunId}",transaction.Id,id,transaction.Type,transaction.Amount,transaction.CurrencyCode,transaction.Description,transaction.MerchantName,JsonSerializer.Serialize(transaction.Movements.Select(x=>new{x.Id,x.AccountId,x.Amount})),transactionReceipt?.Id,run.Id);await telegram.SendAsync(batch.User.TelegramUserId,$"✅ Записано\n\n{operation.Merchant??operation.Description}\n{operation.Amount:0.00} {operation.Currency}",ct);
  }
