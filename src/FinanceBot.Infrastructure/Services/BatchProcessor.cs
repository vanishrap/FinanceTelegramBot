...
public sealed class BatchProcessor(IDbContextFactory<FinanceDbContext> factory, ITelegramClient telegram, IVoiceTranscriptionService transcription, IAiExtractionService extraction, FinanceOptions options, ILogger<BatchProcessor> logger)
{
 ...
 private async Task ProcessAsync(long id,CancellationToken ct)
 {
  ...
  catch(Exception ex) when(ex is not OperationCanceledException) { logger.LogError(ex,"Failed to process InputBatchId={InputBatchId}",id);batch.Status=BatchStatus.Failed;if(run!=null){run.Status=RunStatus.Failed;run.Error=ex.Message;run.CompletedAt=DateTimeOffset.UtcNow;}await db.SaveChangesAsync(CancellationToken.None);return; }
  if(confirmation is not null) { try { await telegram.SendAsync(batch.User.TelegramUserId,confirmation,ct); } catch(Exception ex) when(ex is not OperationCanceledException) { logger.LogError(ex,"Transaction was committed, but Telegram confirmation failed for InputBatchId={InputBatchId}",id); } }
 }
}
