...
public sealed class BatchProcessor(IDbContextFactory<FinanceDbContext> factory, ITelegramClient telegram, IVoiceTranscriptionService transcription, IAiExtractionService extraction, FinanceOptions options, ILogger<BatchProcessor> logger)
{
 ...
 private async Task ProcessAsync(long id,CancellationToken ct)
 {
  ...
  try
  {
   ...
   if(checks.Any(x=>!x.Passed)){batch.Status=BatchStatus.NeedsReview;run.Status=RunStatus.Completed;run.CompletedAt=DateTimeOffset.UtcNow;await db.SaveChangesAsync(ct);await NotifyAsync(batch.User.TelegramUserId,$"⚠️ Не удалось подтвердить запись. Распознано: {operation.Amount:0.00} {operation.Currency}. Запись требует проверки.",ct);return;}
   ...
   Account? selectedAccount=operation.AccountId is long accountId ? await db.Accounts.SingleOrDefaultAsync(x=>x.Id==accountId && x.IsActive,ct) : null;
   if(selectedAccount is null && type is TransactionType.Expense or TransactionType.Income or TransactionType.DebtSettlement)
   {
    selectedAccount=await db.Accounts.SingleOrDefaultAsync(x=>x.OwnerUserId==batch.UserId && x.Name=="Не указан" && x.CurrencyCode==operation.Currency,ct) ?? new Account{Name="Не указан",CurrencyCode=operation.Currency,Type=AccountType.Other,OwnerUserId=batch.UserId,CreatedAt=now};
    if(selectedAccount.Id==0) db.Accounts.Add(selectedAccount);
   }
