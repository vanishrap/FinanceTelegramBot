...
public sealed class BatchProcessor(IDbContextFactory<FinanceDbContext> factory, ITelegramClient telegram, IVoiceTranscriptionService transcription, IAiExtractionService extraction, IAiAnalyticsService analytics, IAnalyticsQueryExecutor queryExecutor, FinanceOptions options, ILogger<BatchProcessor> logger)
{
 ...
 private async Task ProcessAsync(long id,CancellationToken ct)
 {
  ...
  try
  {
   ...
   var accounts=await db.Accounts.Where(x=>x.IsActive&&(x.OwnerUserId==null||x.OwnerUserId==batch.UserId)).Select(x=>new{x.Id,x.Name,x.CurrencyCode}).ToListAsync(ct); var categories=await db.Categories.Where(x=>x.IsActive).Select(x=>new{x.Id,x.Name,x.ParentId,Type=x.Type==CategoryType.Expense?EnumStorage.Expense:EnumStorage.Income}).ToListAsync(ct); var context=JsonSerializer.Serialize(new{accounts,categories,defaultCurrency=options.DefaultCurrency,currentDateTime=DateTimeOffset.UtcNow.ToOffset(MalaysiaOffset),timeZone="Asia/Kuala_Lumpur (UTC+08:00)",enumStorage=EnumStorage.AnalyticsDescription,databaseSchema="Users(Id,TelegramUserId,Name); Accounts(Id,Name,CurrencyCode,OwnerUserId,IsActive,Type TEXT: Bank|Cash|Other); Categories(Id,Name,ParentId,Type TEXT); Transactions(Id,CreatedByUserId,Type TEXT,TransactionDate,CurrencyCode,Amount,Description,MerchantName,CategoryId); AccountMovements(Id,TransactionId,AccountId,Amount); Receipts(Id,TransactionId,MerchantName,ReceiptDate,Subtotal,Tax,ServiceCharge,Discount,Rounding,Total,CurrencyCode); ReceiptItems(Id,ReceiptId,RawName,NormalizedName,Quantity,UnitPrice,BaseAmount,Discount,TaxAllocated,ServiceChargeAllocated,FinalAmount,CategoryId); Debts(Id,CreatedByUserId,Direction TEXT,Counterparty,CurrencyCode,OriginalAmount,Description,CreatedAt,Status TEXT: Open|Settled|Cancelled); DebtPayments(Id,DebtId,TransactionId,Amount,PaidAt)"});
