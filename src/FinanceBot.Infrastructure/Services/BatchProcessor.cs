...
public sealed class BatchProcessor(IDbContextFactory<FinanceDbContext> factory, ITelegramClient telegram, IVoiceTranscriptionService transcription, IAiExtractionService extraction, IAiAnalyticsService analytics, IAnalyticsQueryExecutor queryExecutor, FinanceOptions options, ILogger<BatchProcessor> logger)
{
 ...
 private async Task ProcessAsync(long id,CancellationToken ct)
 {
  ...
  try
  {
   ...
   var prepared=new List<(ExtractedOperation Operation,Category? Category,Account? Account,Account? ToAccount,Transaction? Target,List<ValidationCheck> Checks)>();
   foreach(var operation in operations)
   {
    ...
    prepared.Add((operation,category,account,toAccount,target,checks));
   }
   ...
   else if(saved.Count==1&&saved[0].Operation.Kind=="Correction") confirmation=$"✅ Операция ID {saved[0].Transaction!.Id} исправлена\n\nСумма: {saved[0].Transaction.Amount:0.00} {saved[0].Transaction.CurrencyCode}\nОписание: {saved[0].Transaction.MerchantName??saved[0].Transaction.Description}";
