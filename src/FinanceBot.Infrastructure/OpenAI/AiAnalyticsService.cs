...
public sealed class AiAnalyticsService(HttpClient http, FinanceOptions options, ILogger<AiAnalyticsService> logger) : IAiAnalyticsService
{
    ...
    public async Task<AnalyticsPlan> PlanAsync(AiInput input, CancellationToken ct)
    {
        ...
        var prompt = """
            Decide whether the messages ask a question about the family's shared financial data, rather than describe a new transaction or debt. Every authorized bot user has equal access to the complete shared dataset. Never ask whether the request should cover only the current user or all users, and never filter by CreatedByUserId or TelegramUserId. Questions may have any structure and can request detailed analytics by merchant, category, account, receipt item, debt, or arbitrary periods. If it is a question, produce exactly one read-only SQLite SELECT (a WITH ... SELECT is allowed) using only the schema in context. Join paths are AccountMovements.TransactionId -> Transactions.Id, ExchangeDetails.TransactionId -> Transactions.Id, DebtPayments.DebtId -> Debts.Id, and DebtPayments.TransactionId -> Transactions.Id. For receipt questions, query Receipts and ReceiptItems rather than guessing from Transactions. Use COALESCE(Receipts.ReceiptDate, Transactions.TransactionDate) as the effective receipt date. ReceiptItems.FinalAmount is the paid item amount; group item analytics by the item CategoryId, not the transaction CategoryId. Respect query grain: after joining one transaction to many receipt items or account movements, never SUM Transactions.Amount because it would be duplicated; aggregate ReceiptItems.FinalAmount at item grain or pre-aggregate each child table before joining. Enum values are case-sensitive text: Transactions.Type is exactly Expense, Income, Transfer, BalanceAdjustment, or DebtSettlement; Categories.Type is exactly Expense or Income. Never compare them to lowercase literals. DateTimeOffset values are stored as ISO timestamps with offsets. For Malaysia calendar periods, build explicit +08:00 boundaries from context.currentDateTime and filter with datetime(TransactionDate) >= datetime(start) and datetime(TransactionDate) < datetime(end); never use date(TransactionDate), server-local time, or an inclusive 23:59:59 end. "Последний день" means the current Malaysia calendar day unless the user explicitly asks for the rolling previous 24 hours. Debt Direction=Payable means the family owes Counterparty; Receivable means Counterparty owes the family. Never use PRAGMA or modifying statements. Prefer clear column aliases. Aggregate currencies separately; never add unlike currencies. Do not generate zero-filled rows for every category unless explicitly requested; analytics should be based on matching transactions. Limit detail queries to 200 rows. Automatic query feedback means the prior SQL returned no financial data despite recent shared transactions: diagnose its enum/date/join filters and return corrected SQL. If essential interpretation is ambiguous, return a concise Russian clarificationQuestion and null sql. Requests to correct/delete a transaction or reclassify a debt as an expense are mutations, not questions. If this is a transaction or debt statement, set isQuestion=false and both other fields null.
            """;
        ...
        logger.LogDebug("OpenAI analytics planning request: {Request}", ReadableJson.Serialize(payload));
    ...
    {
        var prompt = $"""
            Answer the user's question about the family's shared finances in Russian using only the supplied query result. All authorized users have equal access to the complete result; never claim that other family users' data is unavailable. Preserve the original conversation context, explain periods and grouping, format money readably, mention when no data was found, and do not expose SQL unless asked. Keep the complete answer below 3500 characters so it fits in one Telegram message.
