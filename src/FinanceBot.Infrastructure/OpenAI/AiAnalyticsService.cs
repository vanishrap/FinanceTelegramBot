...
public sealed class AiAnalyticsService(HttpClient http, FinanceOptions options, ILogger<AiAnalyticsService> logger) : IAiAnalyticsService
{
    public async Task<AnalyticsPlan> PlanAsync(AiInput input, CancellationToken ct)
    {
        ...
        var prompt = """
            Decide whether the messages ask a question about the user's financial data, rather than describe a new transaction. Questions may have any structure and can request detailed analytics by merchant, category, account, receipt item, or arbitrary periods. If it is a question, produce exactly one read-only SQLite SELECT (a WITH ... SELECT is allowed). Use only the schema in context, named parameter $userId, and scope transaction/debt data to that user. Never use PRAGMA or modifying statements. Prefer clear column aliases. Aggregate currencies separately; never add unlike currencies. Limit detail queries to 200 rows. If essential interpretation is ambiguous, return a concise Russian clarificationQuestion and null sql. If this is a transaction, set isQuestion=false and both other fields null.
            """;
    ...
    {
        var prompt = "Answer the user's financial question in Russian using only the supplied query result. Preserve the original conversation context, explain periods and grouping, format money and tables readably, mention when no data was found, and do not expose SQL unless asked. Keep the complete answer below 3500 characters so it fits in one Telegram message.";
