...
public sealed class AiExtractionService(HttpClient http, FinanceOptions options, ILogger<AiExtractionService> logger) : IAiExtractionService
{
 public async Task<string> ExtractAsync(AiInput input, CancellationToken ct)
 {
  ...
  var requestPayload = new { model=options.OpenAiModel, input=new object[]{new{role="system",content=(object)new[]{new{type="input_text",text="Extract one financial operation. Balance statements are BalanceAdjustment, never Income. Never invent account/category IDs. Return schema JSON only."}}},new{role="user",content=(object)content}}, text=new{format=new{type="json_schema",name="finance_operation",strict=true,schema}} };
