...
public sealed class AiExtractionService(HttpClient http, FinanceOptions options, ILogger<AiExtractionService> logger) : IAiExtractionService
{
 public async Task<string> ExtractAsync(AiInput input, CancellationToken ct)
 {
  ...
  var operationSchema = new { type="object", additionalProperties=false, required=new[]{"kind","amount","currency","description","merchant","accountId","toAccountId","receipt","categoryId","transactionDate","clarificationQuestion","debtDirection","counterparty"}, properties=new { kind=new{type="string", @enum=new[]{"Expense","Income","Transfer","BalanceAdjustment","DebtSettlement","DebtCreate","Correction"}}, amount=new{type="number"}, currency=new{type="string"}, description=new{type="string"}, merchant=new{type=new[]{"string","null"}}, accountId=new{type=new[]{"integer","null"}}, toAccountId=new{type=new[]{"integer","null"}}, categoryId=new{type=new[]{"integer","null"}}, transactionDate=new{type=new[]{"string","null"},format="date-time"}, clarificationQuestion=new{type=new[]{"string","null"}}, debtDirection=new{type=new[]{"string","null"},@enum=new object?[]{"Payable","Receivable",null}}, counterparty=new{type=new[]{"string","null"}}, receipt=new{type=new[]{"object","null"}, additionalProperties=false, required=new[]{"subtotal","tax","serviceCharge","discount","rounding","total","items"}, properties=new{subtotal=new{type="number"},tax=new{type="number"},serviceCharge=new{type="number"},discount=new{type="number"},rounding=new{type="number"},total=new{type="number"},items=new{type="array",items=new{type="object",additionalProperties=false,required=new[]{"name","quantity","unitPrice","baseAmount","discount","taxAllocated","serviceChargeAllocated","finalAmount","categoryId","confidence"},properties=new{name=new{type="string"},quantity=new{type="number"},unitPrice=new{type="number"},baseAmount=new{type="number"},discount=new{type="number"},taxAllocated=new{type="number"},serviceChargeAllocated=new{type="number"},finalAmount=new{type="number"},categoryId=new{type=new[]{"integer","null"}},confidence=new{type="number"}}}}}} } };
  ...
  var requestPayload = new { model=options.OpenAiModel, input=new object[]{new{role="system",content=(object)new[]{new{type="input_text",text="Extract every distinct financial operation from the complete conversation. One message may contain multiple expenses, income entries, transfers, or debts: return each as a separate operations array item and never combine their amounts. Later messages answer or correct earlier messages and take precedence; a confirmation to record them separately applies to all previously described operations and is not a reason to ask which one to record first. Balance statements are BalanceAdjustment, never Income. A statement that the user owes someone or someone owes the user is DebtCreate: Payable means the user owes counterparty, Receivable means counterparty owes the user; always extract counterparty and debtDirection. Description must state the actual purpose or counterparty; never replace a missing purpose with generic words such as Expense, Расход, Purchase, or Трата. Select the most specific matching categoryId and an accountId from context for every operation. Never invent IDs. Extract each actual transaction date/time as ISO 8601 when stated; resolve relative dates using the Malaysia currentDateTime in context, otherwise return null so the application uses the current Malaysia time. Put a clarificationQuestion only on the specific operation with a material ambiguity. Do not ask the user to choose between otherwise complete operations. Return schema JSON only."}}},new{role="user",content=(object)content}}, text=new{format=new{type="json_schema",name="finance_operations",strict=true,schema}} };
