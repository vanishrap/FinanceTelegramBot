...
var options=new FinanceOptions { TelegramBotToken=Required(builder.Configuration,"TELEGRAM_BOT_TOKEN"), OpenAiApiKey=Required(builder.Configuration,"OPENAI_API_KEY"), AllowedTelegramUserIds=allowed, OpenAiModel=Value(builder.Configuration,"OPENAI_MODEL","gpt-4.1-mini"), OpenAiTranscriptionModel=Value(builder.Configuration,"OPENAI_TRANSCRIPTION_MODEL","gpt-4o-mini-transcribe"), MessageBatchDelaySeconds=int.Parse(Value(builder.Configuration,"MESSAGE_BATCH_DELAY_SECONDS","5")), DatabasePath=Value(builder.Configuration,"DATABASE_PATH","/data/finance.db"), DefaultCurrency=Value(builder.Configuration,"DEFAULT_CURRENCY","MYR"), ValidationRoundingTolerance=decimal.Parse(Value(builder.Configuration,"VALIDATION_ROUNDING_TOLERANCE","0.05"),System.Globalization.CultureInfo.InvariantCulture) };
...
builder.Services.AddSingleton<MessageBatchService>();builder.Services.AddSingleton<BatchProcessor>();builder.Services.AddHostedService<PollingWorker>();builder.Services.AddHostedService<BatchProcessingWorker>();
