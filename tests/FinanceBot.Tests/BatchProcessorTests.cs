    ...
    public async Task ProcessDueAsync_ReplansEmptyAnalyticsQuery_WhenRecentTransactionsExist()
    {
        ...
        try
        {
            ...
            await using (var db = factory.CreateDbContext())
            {
                ...
                db.Transactions.Add(new Transaction { CreatedByUserId = userId, Type = TransactionType.Expense, TransactionDate = DateTimeOffset.UtcNow.ToOffset(TimeSpan.FromHours(8)), CurrencyCode = "MYR", Amount = 130.50m, Description = "Mercato", CreatedAt = DateTimeOffset.UtcNow, UpdatedAt = DateTimeOffset.UtcNow });
