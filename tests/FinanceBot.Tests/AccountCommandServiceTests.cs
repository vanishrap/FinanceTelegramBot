...
{
    [Fact]
    public async Task AccountAndAccountsCommands_CreateAndListUserAccount()
    {
        ...
        try
        {
            ...
            var service = new AccountCommandService(factory, new FinanceOptions { AllowedTelegramUserIds = [42] });
            ...
            var listed = await service.TryHandleAsync(new TelegramUpdate(2, 11, 42, "Test", "/accounts", null, null), CancellationToken.None);
            ...
            var account = await verification.Accounts.Include(x => x.OwnerUser).SingleAsync(x => x.Name == "Основная карта");
            Assert.Equal(AccountType.Bank, account.Type);
            Assert.Equal(42, account.OwnerUser?.TelegramUserId);
        }
        finally
        {
