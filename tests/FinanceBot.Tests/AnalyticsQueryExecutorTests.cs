    ...
    public async Task ExecuteAsync_ReturnsOnlyRequestedUsersRows()
    {
        ...
        try
        {
            ...
            await using(var db=await factory.CreateDbContextAsync())
            {
                ...
                var json=await new AnalyticsQueryExecutor(factory).ExecuteAsync("SELECT Description, Amount FROM Transactions WHERE CreatedByUserId = $userId", first.Id, CancellationToken.None);
